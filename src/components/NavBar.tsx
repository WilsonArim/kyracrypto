...
const MENU: MenuItem[] = [
  ...
  },
  {
    label: 'DAO',
    submenu: [
      { label: 'EVM agregador Tally', url: 'https://www.tally.xyz/explore', external: true },
      { label: 'SOL agregador Realms DAO', url: 'https://app.realms.today/realms', external: true }
    ],
  },
  { label: 'RWA', path: '/rwa' },
  {
...
export default function NavBar() {
  ...
  return (
    <nav className="navbar-glow w-full flex items-center justify-between gap-8 pt-6 pb-2 relative z-50">
      {/* Espaço reservado para o logo */}
      <div className="w-[96px] min-w-[80px]" />
      {/* Menus centralizados */}
      <div className="flex-1 flex items-center justify-center gap-8">
        {MENU.map((item) => (
          ...
          >
            {item.submenu ? (
              <span className="text-cyan-300 font-bold tracking-wide text-base px-2 cursor-pointer hover:text-[#00BFFF] select-none">
                {item.label}
              </span>
            ) : (
              <Link href={item.path || ''} legacyBehavior>
                <a className="text-cyan-300 font-bold tracking-wide text-base px-2 cursor-pointer hover:text-[#00BFFF] select-none">
                  {item.label}
                </a>
              </Link>
            )}
              ...
              >
                <div className="submenu-content bg-transparent rounded-xl shadow-lg py-2 px-4 border border-cyan-400">
                  {item.submenu.map((sub) => (
                    sub.external ? (
                      <a key={sub.label} href={sub.url} target="_blank" rel="noopener noreferrer" className="text-cyan-300 font-bold tracking-wide text-base py-1 px-2 rounded w-full text-left cursor-pointer hover:bg-[#232136] transition underline flex items-center justify-between">
                        <div>
                      ...
                      <div key={sub.label} className="relative group w-full">
                        <span className="text-cyan-300 font-bold tracking-wide text-base py-1 px-2 rounded w-full text-left cursor-pointer hover:bg-[#232136] transition flex items-center justify-between">
                          {sub.label}
                        ...
                        <div className="submenu-glow absolute left-full top-0 mt-0 ml-2 min-w-[200px] flex flex-col items-center z-50 hidden group-hover:flex">
                          <div className="submenu-content bg-transparent rounded-xl shadow-lg py-2 px-4 border border-cyan-400">
                            {sub.submenu.map((link) => (
                              link.external ? (
                                <a key={link.label} href={link.url} target="_blank" rel="noopener noreferrer" className="text-cyan-300 font-bold tracking-wide text-base py-1 px-2 rounded w-full text-left cursor-pointer hover:bg-[#232136] transition underline flex items-center">
                                ...
                                <Link href={link.url || link.path || ''} key={link.label} legacyBehavior>
                                  <a className="text-cyan-300 font-bold tracking-wide text-base py-1 px-2 rounded w-full text-left cursor-pointer hover:bg-[#232136] transition">
                                    {link.label}
                                  </a>
                                </Link>
                              )
                      ...
                      <Link href={sub.url || sub.path || ''} key={sub.label} legacyBehavior>
                        <a className="text-cyan-300 font-bold tracking-wide text-base py-1 px-2 rounded w-full text-left cursor-pointer hover:bg-[#232136] transition flex items-center justify-between">
                          <div>
                            {sub.icon && <img src={sub.icon} alt={sub.label} className="inline-block w-5 h-5 mr-2 align-middle rounded-full" />}
                            {sub.label}
                          </div>
      ...
      <div className="flex items-center">
        {isAuthenticated && user ? (
          ...
          >
            <button className="text-cyan-300 font-bold tracking-wide text-base px-3 py-2 rounded-lg border border-cyan-400 hover:bg-cyan-700 transition select-none">
          ...
          <button
            ...
            className="text-cyan-300 font-bold tracking-wide text-base px-3 py-2 rounded-lg border border-cyan-400 hover:bg-cyan-700 transition select-none"
          >
            CONECTAR
          </button>
        )}
